...
bool MainWindow::readFile(const QString &fileName)
{
    ...
    while (!reader.atEnd()) {
        if (reader.isStartElement()) {
            if (QString(reader.name().data(), reader.name().size()) == "bookindex") {
...
{
    Q_ASSERT(reader.isStartElement() && QString(reader.name().data(), reader.name().size()) == "bookindex");
    reader.readNext();
    while (!reader.atEnd()) {
        ...
        if (reader.isStartElement()) {
            if (QString(reader.name().data(), reader.name().size()) == "entry") {
...
void MainWindow::readEntryElement(QTreeWidgetItem *parent)
{
    ...
    while (!reader.atEnd()) {
        ...
        if (reader.isStartElement()) {
            ...
            } else if (QString(reader.name().data(), reader.name().size()) == "page") {
